...
## Dernière migration métier

Appliquer également la migration `supabase/migrations/20260913220000_customer_loyalty_payment_offline_hardening.sql` après les migrations précédentes.

Elle finalise :
- mémorisation automatique des clients saisis dans une vente ;
- réutilisation d'un client existant par nom exact ;
- fidélité atomique et calcul correct de la visite offerte ;
- paiement USD/CDF et monnaie rendue ;
- replay Offline sécurisé ;
- statut Livrée ;
- conservation de l'identité locale Offline sans expiration arbitraire de 8 heures.


### Migration 20260913223000
- Ajoute `sync_offline_customer()` pour synchroniser les clients créés hors ligne sans créer de doublons.
- Réutilise un client existant par nom normalisé et conserve les visites/points historiques.

## 10. Création des utilisateurs administrés

La création depuis **Utilisateurs** passe exclusivement par `supabase/functions/manage-users`.
Le navigateur transmet uniquement l'identité et le rôle demandé. Le serveur récupère le `site_id`
du propriétaire/administrateur authentifié et l'injecte dans `app_metadata` avec le rôle demandé.

Le trigger `on_auth_user_created` applique alors ces valeurs : un compte contenant un `site_id`
mais sans rôle valide **échoue** au lieu de devenir `owner`. Le trigger ne crée un nouveau site
que pour une inscription autonome sans `site_id`.

Après la création Auth, la fonction vérifie `profiles.site_id` et `profiles.role`, enregistre les
permissions de menus et l'audit. Si une étape échoue, elle supprime le compte Auth nouvellement
créé (et donc son profil) avant de renvoyer l'erreur. Cela évite le cas « Impossible de créer »
au premier clic suivi de « utilisateur déjà créé » au second clic.

La clé `SUPABASE_SERVICE_ROLE_KEY` est utilisée uniquement dans l'Edge Function et ne doit jamais
être ajoutée à `VITE_*`, au frontend ou au dépôt Git.
