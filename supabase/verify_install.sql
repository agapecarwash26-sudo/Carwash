...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE oid = 'public.handle_new_user()'::regprocedure
      AND prosrc LIKE '%v_requested_role NOT IN%'
  ) THEN
    RAISE EXCEPTION 'Le trigger handle_new_user n''est pas dans sa version stricte.';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'La table user_menu_permissions existe mais aucun index supplémentaire n''a été détecté.';
  END IF;
