...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      try {
        // The auth.users INSERT trigger runs immediately and may provision a
        // profile before this Edge Function gets control back. Some installed
        // database versions intentionally forbid UPDATE of profiles.site_id.
        // Therefore we must NEVER try to repair tenant membership with UPDATE.
        // If the trigger produced a mismatched profile, replace that just-created
        // profile with the authoritative tenant/role from the authenticated caller.
        // The service-role client is used deliberately; browser RLS must never be
        // allowed to choose another tenant.
        ...
        // Never accept the site/role chosen by the Auth trigger as authoritative
        // for an internal staff account. The caller's authenticated profile is
        // the only source of truth for tenant and requested role.
        //
        // IMPORTANT: profiles.site_id is protected against UPDATE by a database
        // trigger. Consequently, when the Auth trigger has already created a
        // mismatched profile, we remove ONLY this brand-new profile and INSERT it
        // again with the correct tenant. This does not move an existing user from
        // one site to another: this code runs only for the newly-created Auth id.
        if (profile.site_id !== requestedSiteId || profile.role !== requestedRole || profile.full_name !== fullName || profile.email !== email) {
          ...
          if (deleteProfileErr) {
            throw new Error(`Impossible de préparer le profil utilisateur : ${deleteProfileErr.message}`);
          }
          ...
          if (recreateErr) throw new Error(`Impossible de créer le profil sur le site du créateur : ${recreateErr.message}`);
          if (!recreated || recreated.site_id !== requestedSiteId || recreated.role !== requestedRole) {
            throw new Error("Le profil utilisateur n'a pas pu être créé sur le site du créateur. Création annulée pour protéger le multi-site.");
          }
        }

        // Keep Auth metadata aligned with the authoritative profile as well.
        const { error: metadataErr } = await supabase.auth.admin.updateUserById(newUserId, {
          user_metadata: { full_name: fullName },
          app_metadata: { site_id: requestedSiteId, role: requestedRole },
        });
        if (metadataErr) throw new Error(`Impossible de finaliser les métadonnées du compte : ${metadataErr.message}`);
