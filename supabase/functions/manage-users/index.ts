...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      const { email, password, full_name, address = "", role, menu_permissions = [] } = body;
      ...
      // The auth.users trigger is authoritative for role/site_id. Verify the
      // resulting profile, then provision permissions and audit as one server flow.
      try {
        ...
        if (!profile || profile.site_id !== callerSiteId || profile.role !== role) {
          throw new Error("Le profil créé ne correspond pas au site ou au rôle demandé.");
        }

        const permissions = Array.isArray(menu_permissions) ? menu_permissions : [];
        if (permissions.length) {
          const rows = permissions.filter((menu_key) => typeof menu_key === "string" && menu_key.trim())
            .map((menu_key) => ({ site_id: callerSiteId, user_id: authData.user.id, menu_key, can_view: true }));
          if (rows.length) {
            const { error: permissionsErr } = await supabase.from("user_menu_permissions").upsert(rows, { onConflict: "site_id,user_id,menu_key" });
            if (permissionsErr) throw permissionsErr;
          }
        }

        const { error: auditErr } = await supabase.from("audit_logs").insert({
          site_id: callerSiteId, actor_id: user.id, action: "create_user", entity_type: "profiles",
          entity_id: authData.user.id,
          details: { created_user_id: authData.user.id, email: email.toLowerCase(), role, site_id: callerSiteId }
        });
        if (auditErr) throw auditErr;
      } catch (provisionErr) {
        // Avoid leaving a successful Auth account without a coherent tenant profile.
