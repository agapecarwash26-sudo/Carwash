...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      try {
        // The auth.users INSERT trigger runs immediately and can provision a
        // profile before this Edge Function gets control back. Therefore the
        // Edge Function is the final authority for users created by an
        // owner/admin: it MUST reconcile the profile to the caller's site and
        // requested role instead of merely rejecting a mismatched profile.
        // This also makes the flow resilient to an older/stale trigger already
        // installed remotely. The service-role client is used here deliberately
        // because browser RLS must never be allowed to choose another tenant.
        ...
        // for an internal staff account. Force both values from the already
        // authenticated caller context. This is the critical multi-site guard.
        if (profile.site_id !== requestedSiteId || profile.role !== requestedRole || profile.full_name !== fullName || profile.email !== email) {
          ...
          if (reconcileErr) throw new Error(`Impossible de rattacher le profil au site du créateur : ${reconcileErr.message}`);
          if (!reconciled || reconciled.site_id !== requestedSiteId || reconciled.role !== requestedRole) {
            throw new Error("Le profil utilisateur n'a pas pu être rattaché au site du créateur. Création annulée pour protéger le multi-site.");
          }
        }
