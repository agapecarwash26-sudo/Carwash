...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      // Verify and repair the provisioning result using the server-derived tenant.
      // Never trust a site_id from the browser.
      ...
      if (profileReadErr || !profile) {
        return json({ error: "Compte Auth créé mais profil non provisionné. Réessayez la récupération administrative; aucun second compte ne doit être créé." }, 500);
      }
      if (profile.site_id !== callerSiteId || profile.role !== role) {
        const { error: repairErr } = await supabase.from("profiles").update({
          email: email.toLowerCase(), full_name, address,
          site_id: callerSiteId, role, updated_at: new Date().toISOString(),
        }).eq("id", authData.user.id);
        if (repairErr) return json({ error: `Compte créé mais profil incohérent: ${repairErr.message}` }, 500);
      } else {
        const { error: updateErr } = await supabase.from("profiles").update({ email: email.toLowerCase(), full_name, address, updated_at: new Date().toISOString() }).eq("id", authData.user.id);
        if (updateErr) return json({ error: `Compte créé mais profil impossible à mettre à jour: ${updateErr.message}` }, 500);
      }

      if (Array.isArray(menuKeys)) {
        const rows = menuKeys.filter((x: unknown) => typeof x === "string" && x.length > 0).map((menu_key: string) => ({ site_id: callerSiteId, user_id: authData.user.id, menu_key, can_view: true }));
        if (rows.length) {
          const { error: menusErr } = await supabase.from("user_menu_permissions").upsert(rows, { onConflict: "site_id,user_id,menu_key" });
          if (menusErr) return json({ error: `Compte créé mais permissions de menu non enregistrées: ${menusErr.message}` }, 500);
        }
      }

      return json({ id: authData.user.id, email, full_name, address, role, site_id: callerSiteId }, 201);
    }
