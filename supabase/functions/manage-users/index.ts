...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      // The database trigger creates the profile and assigns site_id.
      // Do not update profiles here: the security trigger intentionally
      // blocks direct tenant/site changes from this function.
      ...
      if (!createdProfile) {
        throw new Error("Le profil du nouvel utilisateur n'a pas été créé.");
      }
      if (createdProfile.site_id !== callerSiteId) {
        throw new Error("Le nouvel utilisateur n'a pas été rattaché au bon site.");
      }

      return json({
        id: authData.user.id,
        email: createdProfile.email ?? email,
        full_name: createdProfile.full_name ?? full_name,
