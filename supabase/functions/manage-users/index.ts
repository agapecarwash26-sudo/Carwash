...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      const { email, password, full_name, address = "", role, menuKeys = [] } = body;
      ...
      const { data: authData, error: authErr } = await supabase.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        user_metadata: { full_name, address },
      ...
      // The trigger provisions the initial profile. From this point on, every
      // required step is treated as one logical operation. If anything fails,
      // remove the Auth user so a retry cannot hit a half-created account.
      const compensate = async (reason: string) => {
        const { error: deleteErr } = await supabase.auth.admin.deleteUser(newUserId);
        if (deleteErr) console.error("USER_PROVISIONING_COMPENSATION_FAILED", deleteErr.message, reason);
      };
      ...
      if (profileReadErr || !profile) {
        await compensate("profile_missing");
        return json({ error: "Création annulée : le profil utilisateur n'a pas pu être provisionné." }, 500);
      }

      // Always normalize the profile to the server-derived tenant and the
      // role selected by the authorized caller. site_id from the browser is ignored.
      ...
      if (profileUpdateErr) {
        await compensate("profile_update_failed");
        return json({ error: `Création annulée : profil non enregistré (${profileUpdateErr.message}).` }, 500);
      }
      ...
      const { data: verifiedProfile, error: verifyErr } = await supabase
        ...
        .eq("id", newUserId)
        .maybeSingle();
      if (verifyErr || !verifiedProfile || verifiedProfile.site_id !== callerSiteId || verifiedProfile.role !== role) {
        await compensate("profile_verification_failed");
        return json({ error: "Création annulée : le profil ne correspond pas au site et au rôle demandés." }, 500);
      }

      if (Array.isArray(menuKeys)) {
        const rows = menuKeys
          .filter((x: unknown) => typeof x === "string" && x.length > 0)
          .map((menu_key: string) => ({ site_id: callerSiteId, user_id: newUserId, menu_key, can_view: true }));
        if (rows.length) {
          const { error: menusErr } = await supabase.from("user_menu_permissions").upsert(rows, { onConflict: "site_id,user_id,menu_key" });
          if (menusErr) {
            await compensate("menu_permissions_failed");
            return json({ error: `Création annulée : permissions de menu non enregistrées (${menusErr.message}).` }, 500);
          }
        }
      }

      // Explicit audit entry: service-role execution has no auth.uid(), so the
      // actor must be recorded explicitly rather than relying on the row trigger.
      const { error: auditErr } = await supabase.from("audit_logs").insert({
        ...
        entity_type: "user",
        entity_id: newUserId,
        details: { email: email.toLowerCase(), full_name, address, role, site_id: callerSiteId },
      });
      if (auditErr) {
        await compensate("audit_failed");
        return json({ error: `Création annulée : journal d'audit non enregistré (${auditErr.message}).` }, 500);
      }

      return json({ id: newUserId, email: email.toLowerCase(), full_name, address, role, site_id: callerSiteId }, 201);
    }
