...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      // handle_new_user() creates the profile atomically from app_metadata.
      // The update below is only a metadata synchronization and keeps the
      // tenant assignment controlled by the server-side trigger.
      ...
      if (profileErr) {
        // Avoid leaving an Auth account that the UI reports as failed.
      ...
      const { error: auditErr } = await supabase.from("audit_logs").insert({
        ...
        details: { email: email.toLowerCase(), full_name, role },
      });
      if (auditErr) console.error("Audit log failed:", auditErr);

      return json({ id: authData.user.id, email: email.toLowerCase(), full_name, role, site_id: callerSiteId }, 201);
    }
