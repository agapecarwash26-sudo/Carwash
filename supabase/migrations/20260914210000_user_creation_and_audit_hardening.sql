...
BEGIN
  ...
  IF v_site_id IS NULL THEN
    BEGIN
      v_site_id := public.current_user_site_id();
  ...
  VALUES (
    ...
    v_entity_id,
    jsonb_build_object(
      'new', CASE WHEN TG_OP <> 'DELETE' THEN v_row ELSE NULL END,
      'old', v_old
    )
  );
...
BEGIN
  ...
  IF v_requested_site IS NOT NULL THEN
    ...
    EXCEPTION WHEN invalid_text_representation THEN
      RAISE EXCEPTION 'Le site demandé pour ce compte est invalide.';
    END;
    ...
    ELSE
      RAISE EXCEPTION 'Le rôle demandé pour ce compte est invalide.';
    END IF;
  ELSE
