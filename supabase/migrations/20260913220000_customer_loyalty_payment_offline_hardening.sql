-- Customer memory, loyalty import, payment correctness and offline-safe order replay.
...
BEGIN
  ...
  CASE p_operation_type
    WHEN 'VENTE' THEN
      ...
      -- Si le caissier saisit simplement le nom d'un client connu, on réutilise
      -- automatiquement son dossier. Sinon, un nouveau dossier est créé.
      IF customer_id IS NULL AND NULLIF(BTRIM(p_payload->>'customerName'),'') IS NOT NULL THEN
        SELECT * INTO new_customer
        FROM public.customers
        WHERE site_id=current_user_site_id()
          AND LOWER(BTRIM(full_name)) = LOWER(BTRIM(p_payload->>'customerName'))
        ORDER BY created_at
        LIMIT 1
        FOR UPDATE;
        IF FOUND THEN
        ...
        ELSE
          ...
          VALUES(current_user_site_id(),BTRIM(p_payload->>'customerName'),'','')
          RETURNING * INTO new_customer;
          customer_id := new_customer.id;
        END IF;
      ELSIF customer_id IS NOT NULL THEN
      ...
      -- La gratuité est déterminée avant de calculer le montant à payer.
      IF customer_id IS NOT NULL THEN
        ...
        SET visits=visits+1,
            ...
            total_spent=total_spent+CASE WHEN loyalty_is_free THEN 0 ELSE GREATEST(subtotal_usd-discount_amount_usd,0) END,
            updated_at=now()
        WHERE id=customer_id
        RETURNING * INTO new_customer;
      END IF;

      payment_amount := CASE WHEN loyalty_is_free THEN 0 WHEN payment_currency='CDF' THEN GREATEST(subtotal_usd-discount_amount_usd,0)*exchange_rate ELSE GREATEST(subtotal_usd-discount_amount_usd,0) END;
    ...
    WHEN 'STOCK_ADJUST' THEN
      ...
      ELSIF p_payload->>'action'='used_up' THEN
        ...
        INSERT INTO public.stock_movements(product_id,type,quantity,reason,created_by) VALUES(new_product.id,'in',1,'Transfert : +1 stock utilisé / -1 stock de sécurité',auth.uid());
    ...
    WHEN 'REAPPRO' THEN
      ...
      IF p_payload->>'type'='out' THEN
        ...
        SET used_stock=GREATEST(0,used_stock-(p_payload->>'quantity')::numeric),
            current_stock=GREATEST(0,used_stock-(p_payload->>'quantity')::numeric)
    ...
    WHEN 'CLIENT' THEN
      INSERT INTO public.customers(site_id,full_name,phone,email,visits,loyalty_points) VALUES(current_user_site_id(),p_payload->>'full_name',COALESCE(p_payload->>'phone',''),NULLIF(p_payload->>'email',''),GREATEST(COALESCE((p_payload->>'initial_visits')::integer,0),0),GREATEST(COALESCE((p_payload->>'initial_loyalty_points')::integer,0),0)) RETURNING * INTO new_customer;
