-- AquaFlow production offline synchronization
...
CREATE INDEX IF NOT EXISTS offline_operations_user_created_idx ON public.offline_operations(user_id, created_at);
ALTER TABLE public.offline_operations ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS offline_operations_select_own ON public.offline_operations;
CREATE POLICY offline_operations_select_own ON public.offline_operations FOR SELECT TO authenticated USING (user_id = auth.uid());
DROP POLICY IF EXISTS offline_operations_insert_own ON public.offline_operations;
CREATE POLICY offline_operations_insert_own ON public.offline_operations FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid() AND site_id = current_user_site_id());
DROP POLICY IF EXISTS offline_operations_update_own ON public.offline_operations;
CREATE POLICY offline_operations_update_own ON public.offline_operations FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
BEGIN
  ...
  CASE p_operation_type
    ...
    WHEN 'SETTING' THEN
      INSERT INTO public.app_settings(key,value,updated_at) VALUES(p_payload->>'key',p_payload->>'value',now()) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=now();
      result_json := jsonb_build_object('key',p_payload->>'key','value',p_payload->>'value');
    WHEN 'PRODUIT' THEN
      INSERT INTO public.products(name,category,unit,current_stock,minimum_stock,unit_cost,site_id)
      VALUES(p_payload->>'name',p_payload->>'category',p_payload->>'unit',COALESCE((p_payload->>'current_stock')::numeric,0),COALESCE((p_payload->>'minimum_stock')::numeric,0),COALESCE((p_payload->>'unit_cost')::numeric,0),current_user_site_id())
      RETURNING * INTO new_product;
      result_json := to_jsonb(new_product);
    WHEN 'EMPLOYEE' THEN
      INSERT INTO public.employees(full_name,role,phone,is_active,site_id)
      VALUES(p_payload->>'full_name',p_payload->>'role',COALESCE(p_payload->>'phone',''),COALESCE((p_payload->>'is_active')::boolean,true),current_user_site_id())
      RETURNING * INTO new_employee;
      result_json := to_jsonb(new_employee);
    WHEN 'ABONNEMENT' THEN
      INSERT INTO public.subscriptions(customer_id,name,price_usd,allowed_washes,used_washes,starts_at,expires_at,status)
      VALUES((p_payload->>'customer_id')::uuid,p_payload->>'name',COALESCE((p_payload->>'price_usd')::numeric,0),(p_payload->>'allowed_washes')::integer,0,current_date,(p_payload->>'expires_at')::date,'active')
      RETURNING * INTO new_subscription;
      result_json := to_jsonb(new_subscription);
    WHEN 'SITE_UPDATE' THEN
      UPDATE public.sites SET name=p_payload->>'name',address=p_payload->>'address',phone=p_payload->>'phone'
      WHERE id=(p_payload->>'id')::uuid AND id=current_user_site_id() RETURNING * INTO new_site;
      IF NOT FOUND THEN RAISE EXCEPTION 'Site introuvable ou accès refusé.'; END IF;
      result_json := to_jsonb(new_site);
    ELSE
