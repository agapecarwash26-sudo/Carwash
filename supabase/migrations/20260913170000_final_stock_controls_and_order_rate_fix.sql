-- Final repair for stock quick controls and order exchange-rate display.
-- Idempotent: safe to apply to databases where earlier stock migrations were missed.
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS exchange_rate numeric(14,4);
UPDATE public.orders
SET exchange_rate = COALESCE(exchange_rate, (SELECT COALESCE(NULLIF(s.value, '')::numeric, 2850) FROM public.app_settings s WHERE s.site_id = public.orders.site_id AND s.key = 'usd_to_cdf_rate'), 2850)
WHERE exchange_rate IS NULL;
ALTER TABLE public.orders ALTER COLUMN exchange_rate SET DEFAULT 2850;
