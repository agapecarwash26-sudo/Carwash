...
Supabase CLI/Preview uses only `supabase/migrations/`. Migration filenames must start with the exact 14-digit migration version. Do not prefix a migration with a second timestamp and do not use `.sql.sql`. The legacy `db/migrations/` directory is intentionally non-canonical.

If Preview reports `Remote migration versions not found in local migrations directory`, the local filenames do not match versions recorded in the remote migration history. Do not create a random repair migration: reconcile the filenames/history first.
