...
export async function loadAuditLogs(limit = 100): Promise<{ id: string; action: string; entity_type: string; entity_id: string | null; details: Record<string, unknown>; created_at: string }[]> {
  const { data, error } = await supabase.from('audit_logs').select('id, action, entity_type, entity_id, details, created_at').eq('site_id', (await currentActor()).siteId).order('created_at', { ascending: false }).limit(limit);
  if (error) throw error;
...
export async function createUser(input: { email: string; password: string; full_name: string; address?: string; role: string; menu_permissions?: string[] }): Promise<UserAccount> {
  const { data: { session } } = await supabase.auth.getSession();
  if (!session) throw new Error('Non connecté.');
  const apiUrl = `${supabaseUrl}/functions/v1/manage-users`;
  const res = await fetch(apiUrl, {
