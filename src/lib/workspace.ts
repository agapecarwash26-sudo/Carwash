...
export async function loadProducts(): Promise<Product[]> {
  const { data, error } = await supabase
    .from('products')
    .select('id, name, category, unit, current_stock, minimum_stock, used_stock, used_stock_minimum, used_stock_target, security_stock, security_stock_minimum, unit_cost')
    .eq('site_id', (await currentActor()).siteId)
    .order('name');
  if (error) throw error;
...
export async function createProduct(input: { name: string; category: string; unit: string; current_stock: number; minimum_stock: number; used_stock?: number; used_stock_minimum?: number; used_stock_target?: number; security_stock?: number; security_stock_minimum?: number; unit_cost: number }): Promise<Product> {
  ...
  const { data, error } = await supabase.from('products').insert(input).select('id, name, category, unit, current_stock, minimum_stock, used_stock, used_stock_minimum, used_stock_target, security_stock, security_stock_minimum, unit_cost').maybeSingle();
...
export async function adjustStock(productId: string, action: 'used_down' | 'used_up' | 'security_up'): Promise<void> {
  ...
  const result = await executeOrQueue('STOCK_ADJUST', params, async (operationId) => {
    const { error } = await supabase.rpc('process_offline_operation', {
      p_operation_id: operationId,
      p_operation_type: 'STOCK_ADJUST',
      p_payload: params,
    });
    if (error) throw error;
  });
