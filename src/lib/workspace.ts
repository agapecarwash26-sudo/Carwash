...
export async function searchCustomers(query: string): Promise<Customer[]> {
  ...
  if (!q) {
    const cached = await cacheGet<Customer[]>('customers').catch(() => null);
    return (cached ?? []).slice().sort((a,b) => a.full_name.localeCompare(b.full_name, 'fr')).slice(0,10);
  }
  try {
    const { data, error } = await supabase
      .from('customers')
    ...
    const cached = await cacheGet<Customer[]>('customers').catch(() => null);
    const merged = [...result, ...(cached ?? []).filter(c => !result.some(r => r.id === c.id) && c.full_name.toLocaleLowerCase().includes(q))];
    return merged.sort((a,b) => a.full_name.localeCompare(b.full_name, 'fr')).slice(0,10);
  } catch (error) {
    const cached = await cacheGet<Customer[]>('customers').catch(() => null);
    if (cached) return cached.filter(c => c.full_name.toLocaleLowerCase().includes(q)).sort((a,b) => a.full_name.localeCompare(b.full_name, 'fr')).slice(0,10);
    throw error;
  }
}
