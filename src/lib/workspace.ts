...
export async function createOrder(input: { orderNumber: string; customerName: string; vehicleLabel: string; totalUsd: number; paymentMethod: string; service: Service; personalItems?: string; vehicleDetails?: string; isFreeVisit?: boolean; customerId?: string; serviceId?: string }) {
  const params = { orderNumber: input.orderNumber, customerName: input.customerName, vehicleLabel: input.vehicleLabel, totalUsd: input.totalUsd, paymentMethod: input.paymentMethod, serviceId: input.serviceId || input.service.id, serviceName: input.service.name, servicePriceUsd: input.service.price_usd, personalItems: input.personalItems || '', vehicleDetails: input.vehicleDetails || '', isFreeVisit: !!input.isFreeVisit, customerId: input.customerId || '' };
...
export async function loadReportData() {
  const [ordersResult, expensesResult, servicesResult] = await Promise.all([
    supabase.from('orders').select('id, total_usd, status, created_at, payment_method').eq('site_id', (await currentActor()).siteId).order('created_at', { ascending: false }).limit(500),
