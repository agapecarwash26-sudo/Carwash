...
export async function loadWorkspaceData() {
  try {
    ...
    const [ordersResult, servicesResult] = await Promise.all([
      supabase.from('orders').select('id, order_number, customer_id, customer_name, vehicle_label, total_usd, paid_usd, payment_method, status, assigned_employee, arrived_at, created_at, personal_items, vehicle_details').eq('site_id', (await currentActor()).siteId).order('created_at', { ascending: false }).limit(100),
...
export async function createOrder(input: { orderNumber: string; customerName: string; vehicleLabel: string; totalUsd: number; paymentMethod: string; paymentCurrency?: string; subtotalUsd?: number; discountAmountUsd?: number; service: Service; personalItems?: string; vehicleDetails?: string; isFreeVisit?: boolean; customerId?: string; serviceId?: string }) {
  const params = { orderNumber: input.orderNumber, customerName: input.customerName, vehicleLabel: input.vehicleLabel, totalUsd: input.totalUsd, subtotalUsd: input.subtotalUsd ?? input.totalUsd, discountAmountUsd: input.discountAmountUsd ?? 0, paymentMethod: input.paymentMethod, paymentCurrency: input.paymentCurrency || 'USD', serviceId: input.serviceId || input.service.id, serviceName: input.service.name, servicePriceUsd: input.service.price_usd, personalItems: input.personalItems || '', vehicleDetails: input.vehicleDetails || '', isFreeVisit: !!input.isFreeVisit, customerId: input.customerId || '' };
