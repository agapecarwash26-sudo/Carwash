...
async function executeOrQueue<T>(type: string, params: Record<string, unknown>, onlineExecutor: (operationId: string) => Promise<T>): Promise<{ queued: boolean; value?: T; operationId?: string }> {
  ...
  if (!(await getForcedOffline()) && await checkBackend()) {
    try { return { queued: false, value: await onlineExecutor(operationId), operationId }; }
    catch (error) { if (!isTransientNetworkError(error)) throw error; /* The SAME operationId is persisted below if the response/network was lost. */ }
  }
  const actor = await currentActor();
  if (!actor.userId) throw new Error('AUTH_SESSION_EXPIRED: session utilisateur indisponible.');
...
export async function executeOfflineOperation(operation: OfflineOperation): Promise<unknown> {
  const { data, error } = await supabase.rpc('process_offline_operation', { p_operation_id: operation.id, p_operation_type: operation.type, p_payload: operation.params });
...
export async function createProduct(input: { name: string; category: string; unit: string; current_stock: number; minimum_stock: number; unit_cost: number }): Promise<Product> {
  const params = { ...input };
  const result = await executeOrQueue('PRODUIT', params, async (operationId) => {
    const { data, error } = await supabase.rpc('process_offline_operation', { p_operation_id: operationId, p_operation_type: 'PRODUIT', p_payload: params });
    if (error) throw error;
    if (!data) throw new Error("L'article n'a pas pu être ajouté.");
    return data as Product;
  });
  return offlineResult(result.queued, result.operationId, result.value, { id: result.operationId || newOperationId(), ...input } as Product);
}
...
export async function createEmployee(input: { full_name: string; role: string; phone: string }): Promise<Employee> {
  const params = { ...input, is_active: true };
  const result = await executeOrQueue('EMPLOYEE', params, async (operationId) => {
    const { data, error } = await supabase.rpc('process_offline_operation', { p_operation_id: operationId, p_operation_type: 'EMPLOYEE', p_payload: params });
    if (error) throw error;
    if (!data) throw new Error("L'employé n'a pas pu être ajouté.");
    return data as Employee;
  });
  return offlineResult(result.queued, result.operationId, result.value, { id: result.operationId || newOperationId(), ...params, created_at: new Date().toISOString() } as Employee);
}
...
export async function createSubscription(input: { customer_id: string; name: string; price_usd: number; allowed_washes: number | null; expires_at: string }): Promise<Subscription> {
  const params = { ...input };
  const result = await executeOrQueue('ABONNEMENT', params, async (operationId) => {
    const { data, error } = await supabase.rpc('process_offline_operation', { p_operation_id: operationId, p_operation_type: 'ABONNEMENT', p_payload: params });
    if (error) throw error;
    if (!data) throw new Error("L'abonnement n'a pas pu être créé.");
    return data as Subscription;
  });
  return offlineResult(result.queued, result.operationId, result.value, { id: result.operationId || newOperationId(), ...input, used_washes: 0, starts_at: new Date().toISOString().slice(0,10), status: 'active' } as Subscription);
}
...
export async function updateSite(id: string, input: { name: string; address: string; phone: string }): Promise<void> {
  await executeOrQueue('SITE_UPDATE', { id, ...input }, async (operationId) => {
    const { error } = await supabase.rpc('process_offline_operation', { p_operation_id: operationId, p_operation_type: 'SITE_UPDATE', p_payload: { id, ...input } });
    if (error) throw error;
  });
}
...
export async function saveSetting(key: string, value: string): Promise<void> { await executeOrQueue('SETTING',{key,value},async(operationId)=>{const {error}=await supabase.rpc('process_offline_operation',{p_operation_id:operationId,p_operation_type:'SETTING',p_payload:{key,value}});if(error)throw error;}); }
...
export async function createCustomerWithVehicle(input: {
  ...
  const params = { full_name: input.full_name, phone: input.phone, email: input.email || '', plate_number: input.plate_number || '', brand: input.brand || '', model: input.model || '', color: input.color || '', vehicle_type: input.vehicle_type || 'Berline' };
  const result = await executeOrQueue('CLIENT', params, async (operationId) => {
  ...
  return offlineResult(result.queued, result.operationId, result.value, { id: result.operationId || newOperationId(), full_name: input.full_name, phone: input.phone, email: input.email || null, total_spent: 0, visits: 0, loyalty_points: 0, created_at: new Date().toISOString(), vehicle_label: input.plate_number || input.brand || input.model ? `${input.brand || ''} ${input.model || ''} · ${input.plate_number || ''}`.trim() : '—' } as Customer);
}
