...
export async function executeOfflineOperation(operation: OfflineOperation): Promise<unknown> {
  ...
  if (operation.type === 'CLIENT') {
    ...
    if (response.error && /401|jwt|token|unauthorized|not authenticated/i.test(response.error.message || '')) {
      const refreshed = await supabase.auth.refreshSession().catch(() => ({ data: { session: null }, error: null }));
      if (refreshed.data.session?.user) response = await runCustomer();
    }
...
export async function createOrder(input: { orderNumber: string; customerName: string; vehicleLabel: string; totalUsd: number; paymentMethod: string; paymentCurrency?: string; subtotalUsd?: number; discountAmountUsd?: number; amountTendered?: number; changeAmount?: number; service: Service; personalItems?: string; vehicleDetails?: string; isFreeVisit?: boolean; customerId?: string; serviceId?: string }) {
