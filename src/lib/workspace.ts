...
export async function executeOfflineOperation(operation: OfflineOperation): Promise<unknown> {
  ...
  if (operation.type === 'CONTROLE') {
    ...
    if (localOrderId) {
      ...
      const serverOrderId = localCreate?.type === 'VENTE'
...
export async function updateCustomer(id: string, input: { full_name: string; phone: string; email?: string }): Promise<Customer> {
  const { data, error } = await supabase.from('customers').update({ full_name: input.full_name.trim(), phone: input.phone.trim(), email: input.email?.trim() || null, updated_at: new Date().toISOString() }).eq('id', id).eq('site_id', (await currentActor()).siteId).select('id, full_name, phone, email, total_spent, visits, loyalty_points, created_at').maybeSingle();
  if (error) throw error;
...
export async function createCustomerWithVehicle(input: {
  full_name: string; phone: string; email?: string; plate_number?: string; brand?: string; model?: string; color?: string; vehicle_type?: string;
}): Promise<Customer> {
  const params = { ...input, email: input.email || '', plate_number: input.plate_number || '', brand: input.brand || '', model: input.model || '', color: input.color || '', vehicle_type: input.vehicle_type || 'Berline' };
  ...
  return offlineResult(result.queued, result.operationId, result.value, { id: result.operationId || newOperationId(), full_name: input.full_name, phone: input.phone, email: input.email || null, total_spent: 0, visits: 0, loyalty_points: 0, created_at: new Date().toISOString() } as Customer);
}
