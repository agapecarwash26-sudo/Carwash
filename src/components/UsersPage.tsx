...
import { Check, KeyRound, MoreHorizontal, Plus, Search, ShieldCheck, Trash2, UserPlus, X, Settings2 } from 'lucide-react';
import { createUser, deleteUser, loadUsers, loadUserMenuPermissions, saveUserMenuPermissions, updateEmployeePassword, updateUserRole } from '@/lib/workspace';
...
export function UsersPage({ currentUserId }: { currentUserId: string }) {
  ...
  const [passwordTarget, setPasswordTarget] = useState<{ id: string; name: string } | null>(null); const [permissionTarget, setPermissionTarget] = useState<UserAccount | null>(null);
  ...
  return <PageShell title="Utilisateurs & rôles" subtitle="Gérez les comptes, les accès et les permissions de votre équipe." action="Nouvel utilisateur" onAction={() => setShowNew(true)}>
        ...
        <div className="table-scroll w-full"><table className="min-w-[640px] text-left">
          ...
          <tbody>{filtered.map(u => <tr key={u.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/70">
            ...
            <td className="px-5 py-4 text-right"><div className="relative flex justify-end gap-1">
              <button onClick={() => setEditingRole({ id: u.id, role: u.role })} className="icon-button" title="Changer le rôle"><ShieldCheck size={16} /></button><button onClick={() => setPermissionTarget(u)} className="icon-button text-violet-600 hover:bg-violet-50" title="Choisir les menus"><Settings2 size={16}/></button>
    ...
    {passwordTarget && <ChangePasswordModal userName={passwordTarget.name} onClose={() => setPasswordTarget(null)} onSubmit={pwd => handlePasswordChange(passwordTarget.id, pwd)} />} {permissionTarget && <PermissionModal user={permissionTarget} onClose={()=>setPermissionTarget(null)}/>}
  </PageShell>;
}

const MENU_OPTIONS=[['dashboard','Tableau de bord'],['orders','Ventes & commandes'],['queue','File de lavage'],['customers','Clients & véhicules'],['services','Services & tarifs'],['cash','Caisse'],['inventory','Stocks'],['expenses','Dépenses'],['employees','Équipe'],['subscriptions','Abonnements'],['appointments','Rendez-vous'],['complaints','Réclamations'],['notifications','Notifications'],['reports','Rapports'],['audit','Journal d’audit'],['users','Utilisateurs'],['settings','Configuration']] as const;
function PermissionModal({user,onClose}:{user:UserAccount;onClose:()=>void}){const [selected,setSelected]=useState<string[]>([]);const [loading,setLoading]=useState(true);const [saving,setSaving]=useState(false);const [error,setError]=useState('');useEffect(()=>{loadUserMenuPermissions(user.id).then(v=>setSelected(v.length?v:MENU_OPTIONS.map(x=>x[0]))).catch(e=>setError(e instanceof Error?e.message:'Erreur')).finally(()=>setLoading(false));},[user.id]);const toggle=(id:string)=>setSelected(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id]);async function save(){setSaving(true);try{await saveUserMenuPermissions(user.id,selected);onClose();}catch(e){setError(e instanceof Error?e.message:'Erreur');}finally{setSaving(false);}}return <Modal title={`Menus accessibles — ${user.full_name}`} onClose={onClose}><div className="space-y-3">{loading?<Empty text="Chargement…"/>:<div className="grid grid-cols-2 gap-2">{MENU_OPTIONS.map(([id,label])=><label key={id} className="flex items-center gap-2 rounded-lg border border-slate-200 p-2 text-xs font-semibold"><input type="checkbox" checked={selected.includes(id)} onChange={()=>toggle(id)}/>{label}</label>)}</div>}{error&&<div className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{error}</div>}<button onClick={save} disabled={loading||saving} className="primary-button w-full"><Check size={16}/>{saving?'Enregistrement…':'Enregistrer les accès'}</button></div></Modal>;}

function NewUserModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [name, setName] = useState(''); const [address, setAddress] = useState(''); const [email, setEmail] = useState(''); const [password, setPassword] = useState(''); const [role, setRole] = useState<Role>('operator'); const [selectedMenus,setSelectedMenus]=useState<string[]>(['dashboard','orders','queue','notifications']); const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  async function submit(e: React.FormEvent) {
    e.preventDefault(); setSaving(true); setError('');
    try { const created=await createUser({ email, password, full_name: name, address, role, menu_permissions: selectedMenus }); onCreated(); } catch (e) { setError(e instanceof Error ? e.message : 'Impossible de créer l\'utilisateur.'); } finally { setSaving(false); }
  }
  return <Modal title="Nouvel utilisateur" onClose={onClose}><form onSubmit={submit} className="space-y-4">
    ...
    <div><label className="label">Adresse</label><input className="input" value={address} onChange={e => setAddress(e.target.value)} /></div><div><label className="label">Adresse e-mail</label><input className="input" type="email" value={email} onChange={e => setEmail(e.target.value)} required /></div>
    ...
    <div><label className="label">Rôle</label><select className="input" value={role} onChange={e => setRole(e.target.value as Role)}><option value="admin">Administrateur</option><option value="manager">Manager</option><option value="cashier">Caissier</option><option value="operator">Opérateur</option><option value="stock_manager">Gestionnaire de stock</option></select></div><div><label className="label">Menus accessibles</label><div className="grid grid-cols-2 gap-2 rounded-xl border border-slate-200 p-3">{MENU_OPTIONS.map(([id,label])=><label key={id} className="flex items-center gap-2 text-xs"><input type="checkbox" checked={selectedMenus.includes(id)} onChange={()=>setSelectedMenus(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id])}/>{label}</label>)}</div></div>
