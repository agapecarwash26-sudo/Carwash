...
function EditCustomerModal({ customer, onClose, onSaved }: { customer: Customer; onClose: () => void; onSaved: () => void }) {
  const [name, setName] = useState(customer.full_name); const [phone, setPhone] = useState(customer.phone); const [email, setEmail] = useState(customer.email || ''); const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  async function submit(e: React.FormEvent) { e.preventDefault(); setSaving(true); setError(''); try { await updateCustomer(customer.id, { full_name: name, phone, email }); onSaved(); } catch (err) { setError(err instanceof Error ? err.message : 'Impossible de modifier le client.'); } finally { setSaving(false); } }
  return <Modal title="Modifier le client" onClose={onClose}><form onSubmit={submit} className="space-y-4"><div><label className="label">Nom complet</label><input className="input" value={name} onChange={e => setName(e.target.value)} required /></div><div><label className="label">Téléphone</label><input className="input" value={phone} onChange={e => setPhone(e.target.value)} /></div><div><label className="label">E-mail</label><input className="input" type="email" value={email} onChange={e => setEmail(e.target.value)} /></div>{error && <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">{error}</div>}<button className="primary-button h-11 w-full" disabled={saving}>{saving ? 'Enregistrement…' : 'Enregistrer'}</button></form></Modal>;
}
      ...
      {loading ? <Empty text="Chargement du stock…" /> : filtered.length === 0 ? <Empty text="Aucun article trouvé." /> : (
        <div className="divide-y divide-slate-50">{filtered.map(p => {
          ...
          return <div key={p.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
            ...
            <div className="flex items-center gap-1"><button onClick={() => quickAdjust(p.id, 'used_down', refresh)} className="icon-button" title="Diminuer le stock utilisé de 1 unité"><ArrowDownLeft size={18} /></button><button onClick={() => quickAdjust(p.id, 'used_up', refresh)} className="icon-button" title="+1 utilisé / -1 sécurité (disponible au seuil critique)" disabled={!usedCritical || security <= 0}><ArrowUpRight size={18} /></button><button onClick={() => quickAdjust(p.id, 'security_up', refresh)} className="icon-button" title="+1 stock de sécurité"><ArrowUpRight size={18} /></button><button className="icon-button" title="Plus d'options"><MoreHorizontal size={18} /></button></div>
          </div>;
        })}</div>
      )}
    </div>
...
async function quickAdjust(productId: string, action: 'used_down' | 'used_up' | 'security_up', after: () => void) {
  try { await adjustStock(productId, action); after(); } catch (error) { console.error('Erreur stock:', error); window.alert(error instanceof Error ? error.message : 'Impossible de modifier le stock.'); }
}
