...
function NewExpenseModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault(); setSaving(true); setError('');
    try { await createExpense({ description: desc, category, amount: Number(amount), currency: 'USD' }); onCreated(); } catch (error) { setError(error instanceof Error ? error.message : 'Impossible d’enregistrer la dépense.'); } finally { setSaving(false); }
  }
