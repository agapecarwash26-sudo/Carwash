...
import {
  ...
  recordStockMovement, refillUsedStock, updateExpenseStatus, updateCustomer, setEmployeeActive,
} from '@/lib/workspace';
...
export function ConnectedInventoryPage() {
  ...
  const stockValue = items.reduce((s, p) => s + (Number(p.used_stock)+Number(p.security_stock))*Number(p.unit_cost), 0);
    ...
    <div className="card overflow-hidden">
      ...
      {loading ? <Empty text="Chargement du stock…" /> : filtered.length === 0 ? <Empty text="Aucun article trouvé." /> : (
        <div className="divide-y divide-slate-50">{filtered.map(p => {
          const used=Number(p.used_stock); const security=Number(p.security_stock); const usedMin=Number(p.used_stock_minimum); const securityMin=Number(p.security_stock_minimum); const target=Number(p.used_stock_target); const critical=used<=usedMin || security<=securityMin; const level=Math.min(100,Math.round((used/(target||1))*100));
          return <div key={p.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
            ...
            <div className="w-full sm:w-64"><div className="mb-2 flex items-start justify-between gap-3 text-xs"><div><div className="font-bold text-slate-700">Stock utilisé: {used} {p.unit}</div><div className="mt-0.5 text-slate-400">Cible de réappro: {target} {p.unit}</div></div><div className="text-right"><div className={`font-semibold ${security<=securityMin ? 'text-rose-600' : 'text-slate-600'}`}>Sécurité: {security} {p.unit}</div><div className="mt-0.5 text-slate-400">Seuil: {securityMin} {p.unit}</div></div></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${used<=usedMin ? 'bg-rose-500' : level > 60 ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${level}%` }} /></div><div className="mt-2 text-[11px] text-slate-400">Le stock de sécurité sert de réserve pour réapprovisionner le stock utilisé.</div></div>
            ...
            <div className="flex flex-wrap gap-1"><button onClick={() => quickRefill(p.id, refresh)} className="secondary-button px-2 py-1 text-[11px]" title="Transférer automatiquement la réserve vers le stock utilisé jusqu'à la cible">Réappro depuis sécurité</button><button onClick={() => quickMove(p.id, 'in', refresh)} className="icon-button" title="Réceptionner dans le stock de sécurité"><ArrowDownLeft size={16} /></button><button onClick={() => quickMove(p.id, 'out', refresh)} className="icon-button" title="Consommer le stock utilisé"><ArrowUpRight size={16} /></button><button className="icon-button"><MoreHorizontal size={18} /></button></div>
          </div>;
        })}</div>
      )}
    </div>
...
async function quickRefill(productId: string, after: () => void) { try { await refillUsedStock(productId); after(); } catch { /* ignore */ } }
...
function NewProductModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [name, setName] = useState(''); const [category, setCategory] = useState('Consommable'); const [unit, setUnit] = useState('unité'); const [stock, setStock] = useState('0'); const [min, setMin] = useState('0'); const [target, setTarget] = useState('0'); const [security, setSecurity] = useState('0'); const [securityMin, setSecurityMin] = useState('0'); const [cost, setCost] = useState('0'); const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  async function submit(e: React.FormEvent) {
    e.preventDefault(); setSaving(true); setError('');
    try { await createProduct({ name, category, unit, current_stock: Number(stock), minimum_stock: Number(min), used_stock:Number(stock), used_stock_minimum:Number(min), used_stock_target:Number(target)||Number(stock), security_stock:Number(security), security_stock_minimum:Number(securityMin), unit_cost:Number(cost) }); onCreated(); } catch { setError('Impossible d’ajouter l’article.'); } finally { setSaving(false); }
  }
  return <Modal title="Nouvel article" onClose={onClose}><form onSubmit={submit} className="space-y-4"><div><label className="label">Nom</label><input className="input" value={name} onChange={e => setName(e.target.value)} required /></div><div className="grid grid-cols-2 gap-3"><div><label className="label">Catégorie</label><input className="input" value={category} onChange={e => setCategory(e.target.value)} /></div><div><label className="label">Unité</label><input className="input" value={unit} onChange={e => setUnit(e.target.value)} /></div></div><div className="grid grid-cols-2 gap-3"><div><label className="label">Stock utilisé</label><input className="input" type="number" value={stock} onChange={e => setStock(e.target.value)} /></div><div><label className="label">Seuil critique utilisé</label><input className="input" type="number" value={min} onChange={e => setMin(e.target.value)} /></div><div><label className="label">Cible du stock utilisé</label><input className="input" type="number" min="0" value={target} onChange={e=>setTarget(e.target.value)}/><div className="mt-1 text-[11px] text-slate-400">Niveau vers lequel la réserve réapprovisionne automatiquement le stock utilisé.</div></div><div><label className="label">Stock de sécurité (réserve)</label><input className="input" type="number" value={security} onChange={e=>setSecurity(e.target.value)}/></div><div><label className="label">Seuil sécurité</label><input className="input" type="number" value={securityMin} onChange={e=>setSecurityMin(e.target.value)}/></div><div><label className="label">Coût unit.</label><input className="input" type="number" value={cost} onChange={e => setCost(e.target.value)} /></div></div>{error && <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">{error}</div>}<button className="primary-button h-11 w-full" disabled={saving}><Check size={17} /> {saving ? 'Enregistrement…' : 'Ajouter'}</button></form></Modal>;
}
