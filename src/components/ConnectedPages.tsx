      ...
      {loading ? <Empty text="Chargement du stock…" /> : filtered.length === 0 ? <Empty text="Aucun article trouvé." /> : (
        <div className="divide-y divide-slate-50">{filtered.map(p => {
          ...
          return <div key={p.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
            ...
            <div className="w-full sm:w-48"><div className="mb-2 flex justify-between text-xs"><span className="font-semibold text-slate-700">{used} {p.unit}</span><span className="text-slate-400">cible {target} {p.unit}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${used<=usedMin ? 'bg-rose-500' : level > 60 ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${level}%` }} /></div><div className={`mt-2 text-[11px] ${security<=securityMin ? 'font-bold text-rose-600' : 'text-slate-400'}`}>Sécurité: {security} {p.unit} · seuil {securityMin}</div></div>
            ...
            <div className="flex gap-1"><button onClick={() => quickRefill(p.id, refresh)} className="secondary-button px-2 py-1 text-[11px]" title="Compléter le stock utilisé depuis la sécurité">Réappro auto</button><button onClick={() => quickMove(p.id, 'in', refresh)} className="icon-button" title="Entrée"><ArrowDownLeft size={16} /></button><button onClick={() => quickMove(p.id, 'out', refresh)} className="icon-button" title="Sortie"><ArrowUpRight size={16} /></button><button className="icon-button"><MoreHorizontal size={18} /></button></div>
          </div>;
        })}</div>
      )}
    </div>
