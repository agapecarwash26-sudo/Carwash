      ...
      {loading ? <Empty text="Chargement du stock…" /> : filtered.length === 0 ? <Empty text="Aucun article trouvé." /> : (
        <div className="divide-y divide-slate-50">{filtered.map(p => {
          ...
          return <div key={p.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
            ...
            <div className="flex items-center gap-1"><button onClick={() => quickAdjust(p.id, 'used_down', refresh)} className="icon-button" title="Diminuer le stock utilisé de 1 unité"><ArrowDownLeft size={18} /></button><button onClick={() => quickAdjust(p.id, 'used_up', refresh)} className="icon-button" title="Ajouter 1 unité au stock utilisé depuis la réserve de sécurité" disabled={security <= 0}><ArrowUpRight size={18} /></button><button onClick={() => quickAdjust(p.id, 'security_up', refresh)} className="icon-button" title="Augmenter le stock de sécurité de 1 unité"><ArrowUpRight size={18} /></button><button className="icon-button" title="Plus d'options"><MoreHorizontal size={18} /></button></div>
          </div>;
        })}</div>
      )}
    </div>
