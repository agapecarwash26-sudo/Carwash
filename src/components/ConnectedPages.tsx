...
import {
  ...
  adjustStock, loadSettings, updateExpenseStatus, updateCustomer, setEmployeeActive,
} from '@/lib/workspace';
...
export function ConnectedExpensesPage() {
  ...
  useEffect(() => { refresh(); loadSettings().then(s => setExchangeRate(Number(s.exchange_rate || s.usd_to_cdf || 2800) || 2800)).catch(() => {}); }, []);
  ...
  return <PageShell title="Dépenses" subtitle="Suivez les sorties et gardez le contrôle sur vos coûts." action="Nouvelle dépense" onAction={() => setShowNew(true)}>
    <div className="grid gap-4 sm:grid-cols-3">
      <StatCard label="Dépenses enregistrées" value={`${formatUsd(totalMonth)} · ${formatCdf(totalMonth * exchangeRate)}`} icon={ArrowUpRight} tone="bg-rose-50 text-rose-600" />
      <StatCard label="En attente de validation" value={`${formatUsd(pending)} · ${formatCdf(pending * exchangeRate)}`} icon={Clock3} tone="bg-amber-50 text-amber-600" />
