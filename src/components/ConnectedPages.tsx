...
export function ConnectedInventoryPage() {
  ...
  return <PageShell title="Stocks" subtitle="Utilisez le stock en 1 clic : ↙️ consommer, ↗️ réapprovisionner depuis la sécurité, ↗️ ajouter à la réserve." action="Ajouter un article" onAction={() => setShowNew(true)}>
    <div className="grid gap-4 sm:grid-cols-3">
    ...
    <div className="rounded-xl border border-cyan-100 bg-cyan-50 p-4 text-sm text-cyan-900">
      ...
      <div className="mt-1 text-xs leading-5">Le <b>stock utilisé</b> est celui consommé au quotidien. Le <b>stock de sécurité</b> est la réserve. Quand le stock utilisé atteint son seuil critique, utilisez ↗️ pour transférer <b>1 unité</b> de la réserve vers le stock utilisé. Il n'y a plus de « cible » à gérer.</div>
    </div>
    {error && <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{error}</div>}
    <div className="card overflow-hidden">
...
async function quickAdjust(productId: string, action: 'used_down' | 'used_up' | 'security_up', after: () => void) {
  try { await adjustStock(productId, action); after(); } catch { /* ignore */ }
}
...
function NewProductModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault(); setSaving(true); setError('');
    try { await createProduct({ name, category, unit, current_stock: Number(stock), minimum_stock: Number(min), used_stock:Number(stock), used_stock_minimum:Number(min), used_stock_target:Number(stock), security_stock:Number(security), security_stock_minimum:Number(securityMin), unit_cost:Number(cost) }); onCreated(); } catch { setError('Impossible d’ajouter l’article.'); } finally { setSaving(false); }
  }
