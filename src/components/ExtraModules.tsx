...
import { loadAuditLogs, loadOrderItems, loadReportData, loadSites, loadProfileName } from '@/lib/workspace';
...
function encodeThermalTicket(order: Order, company: Site | null, items: OrderItem[] = [], cashier: string | null = null): Uint8Array {
  ...
  const money = (value: number, currency = 'USD') => `${Number(value || 0).toLocaleString('fr-FR', { maximumFractionDigits: 2 })} ${currency}`;
  ...
  const text = [
    ...
    line, '\x1B\x61\x01', 'Merci de votre confiance !', '', '', '', '', '\x1B\x64\x05', '\x1D\x56\x00',
  ].join('\n');
  // ESC/POS printers commonly expect a single-byte code page, not UTF-8.
  // UTF-8 makes accents and symbols print as corrupted characters on P5_30D8.
  const cp437: Record<string, number> = {
    'à': 0x85, 'â': 0x83, 'ä': 0x84, 'ç': 0x87, 'é': 0x82, 'è': 0x8a,
    'ê': 0x88, 'ë': 0x89, 'î': 0x8c, 'ï': 0x8b, 'ô': 0x93, 'ö': 0x94,
    'ù': 0x97, 'û': 0x96, 'ü': 0x81, 'À': 0xb7, 'Ç': 0x80, 'É': 0x90,
    'È': 0xd4, 'Ê': 0x92, 'Ô': 0xe2, 'Ù': 0xe9, '€': 0xd5,
    '−': 0x2d, '→': 0x3e, '←': 0x3c, '·': 0x2e,
  };
  const bytes: number[] = [0x1b, 0x74, 0x00]; // ESC t 0: CP437-compatible default
  for (const char of text) {
    if (char === '\n') { bytes.push(0x0a); continue; }
    const code = char.charCodeAt(0);
    bytes.push(code < 0x80 ? code : (cp437[char] ?? 0x3f));
  }
    ...
    <div className="space-y-4">
      ...
      {loading ? <Empty text="Chargement des articles…" /> : items.length === 0 ? <Empty text="Aucun article détaillé." /> : (
        ...
        <div className="table-scroll"><table className="w-full text-left text-sm"><thead><tr className="border-b border-slate-100 text-[10px] uppercase text-slate-400"><th className="pb-2">Service</th><th className="pb-2 text-center">Qté</th><th className="pb-2 text-right">P.U. USD</th><th className="pb-2 text-right">P.U. CDF</th><th className="pb-2 text-right">Total USD</th><th className="pb-2 text-right">Total CDF</th></tr></thead>
          <tbody>{items.map(item => <tr key={item.id} className="border-b border-slate-50 last:border-0"><td className="py-2 font-semibold text-slate-700">{item.service_name}</td><td className="py-2 text-center text-slate-500">{item.quantity}</td><td className="py-2 text-right text-slate-600">{formatUsd(Number(item.unit_price_usd))}</td><td className="py-2 text-right text-slate-600">{Math.round(Number(item.unit_price_usd) * Number(order.exchange_rate || 2850)).toLocaleString('fr-FR')} CDF</td><td className="py-2 text-right font-bold text-slate-800">{formatUsd(Number(item.unit_price_usd) * item.quantity)}</td><td className="py-2 text-right font-bold text-slate-800">{Math.round(Number(item.unit_price_usd) * item.quantity * Number(order.exchange_rate || 2850)).toLocaleString('fr-FR')} CDF</td></tr>)}</tbody>
          </table></div>
      ...
      <button onClick={() => printP5_30D8(encodeThermalTicket(order, company, items, cashier)).catch(error => window.alert(error instanceof Error ? error.message : 'Impression Bluetooth impossible.'))} className="secondary-button h-11 w-full">Imprimer sur P5_30D8 (Bluetooth)</button>
    </div>
  </Modal>;
}
