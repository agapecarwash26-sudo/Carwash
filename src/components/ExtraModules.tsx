...
export function ReceiptModal({ order, onClose }: { order: Order; onClose: () => void }) {
  ...
  const [company, setCompany] = useState<Site | null>(null);

  useEffect(() => { Promise.all([loadOrderItems(order.id),loadSites()]).then(([data,sites])=>{setItems(data);setCompany(sites[0]??null);setLoading(false);}).catch(()=>setLoading(false)); }, [order.id]);
