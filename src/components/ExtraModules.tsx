...
function encodeThermalTicket(order: Order, company: Site | null): Uint8Array {
  const text = `\x1B@${company?.name || 'AquaFlow Car Wash'}\nReçu: ${order.order_number}\nClient: ${order.customer_name}\nTotal: ${formatUsd(order.total_usd)}\n\nMerci de votre confiance !\n\n`;
  return new TextEncoder().encode(text + '\x1D\x56\x00');
}
...
export function ReceiptModal({ order, onClose }: { order: Order; onClose: () => void }) {
  ...
  return <Modal title={`Reçu ${order.order_number}`} onClose={onClose}>
    <div className="space-y-4">
      ...
      <button onClick={() => printP5_30D8(encodeThermalTicket(order, company)).catch(error => window.alert(error instanceof Error ? error.message : 'Impression Bluetooth impossible.'))} className="secondary-button h-11 w-full">Imprimer sur P5_30D8 (Bluetooth)</button>
    </div>
  </Modal>;
}
...
export function AuditPage() {
  ...
  return <div className="space-y-6">
    ...
    {selectedLog && <Modal title="Détails de l’opération" onClose={() => setSelectedLog(null)}><div className="space-y-3 text-sm">{Object.entries({ action: selectedLog.action, module: selectedLog.entity_type, id: selectedLog.entity_id || "—", utilisateur: selectedLog.actor_name || selectedLog.actor_id || "Système", date: new Date(selectedLog.created_at).toLocaleString("fr-FR"), détails: selectedLog.details }).map(([k,v]) => <div key={k} className="rounded-xl bg-slate-50 p-3"><div className="text-xs font-semibold uppercase text-slate-400">{k}</div><div className="mt-1 break-words font-medium text-slate-700">{typeof v === "object" ? JSON.stringify(v, null, 2) : String(v)}</div></div>)}</div></Modal>}
  </div>;
}
